...
def eval_answer(prediction, gold):
    ...
    # acc
    acc = calculate_acc(prediction, gold)
    return em, f1, prec, recall, acc


def update_answer(metrics, prediction, golds):
    max_em, max_f1, max_prec, max_recall, max_acc = 0, 0, 0, 0, 0

    for gold in golds:
        em, f1, prec, recall, acc = eval_answer(prediction, gold)
    ...
    return max_em, max_prec, max_recall, max_acc
...
def eval(prediction_file, gold_file, alias_file):
    ...
    metrics = {'em': 0, 'f1': 0, 'prec': 0, 'recall': 0,
        ...
        'joint_em': 0, 'joint_f1': 0, 'joint_prec': 0, 'joint_recall': 0, 'acc':0}
    ...
    for dp in gold:
        ...
        if cur_id not in prediction['answer']:
            ...
            em, prec, recall, acc = update_answer(

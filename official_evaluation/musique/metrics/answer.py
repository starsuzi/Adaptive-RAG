...
class AnswerMetric(Metric):
    ...
    def __call__(
        ...
        acc_scores = metric_max_over_ground_truths(
            calculate_acc, predicted_answer, ground_truth_answers
        )
    ...
    def get_metric(self, reset: bool = False) -> Tuple[float, float]:
        ...
        acc_score = self._total_acc / self._count if self._count > 0 else 0
        if reset:
            self.reset()
        return exact_match, f1_score, acc_score

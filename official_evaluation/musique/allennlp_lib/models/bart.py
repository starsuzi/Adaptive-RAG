...
class Bart(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Extend bart embeddings automatically (This wasn't part of the original bart.py)
        if self.bart.get_input_embeddings().num_embeddings != len(
            self._indexer._tokenizer
        ):
            self.bart.resize_token_embeddings(len(self._indexer._tokenizer))
